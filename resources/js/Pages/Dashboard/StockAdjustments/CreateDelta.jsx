...
function ItemRow({ item, idx, allItems, usedItemIds, items, setData, errors, onFetchQty, removeItem, updateItem, loadingIdx }) {
    const diff    = parseInt(item.delta_quantity) || 0;
    const finalQty = (parseInt(item.system_quantity) || 0) + diff;
    ...
    return (
        <div className="p-3 sm:p-4 bg-slate-50 dark:bg-slate-800/40 rounded-xl space-y-3 border border-slate-100 dark:border-slate-700/50">
                ...
                <div className="w-28 sm:w-32 shrink-0">
                    ...
                    <select
                        ...
                        onChange={(e) => setData("items", items.map((it, i) =>
                            i === idx ? { ...it, item_type: e.target.value, item_id: "", system_quantity: 0, delta_quantity: "", unit_cost: 0.0 } : it
                        ))}
                    ...
                    <SearchSelect
                        ...
                        onChange={(v) => {
                            setData("items", items.map((it, i) =>
                                i === idx ? { ...it, item_id: v, system_quantity: 0, delta_quantity: "", unit_cost: 0.0 } : it
                            ));
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
                ...
                <div>
                    <label className="block text-xs font-bold text-slate-500 mb-1">Delta Qty (+ / -) *</label>
                    <input
                        type="number"
                        step="1"
                        value={item.delta_quantity}
                        onChange={(e) => updateItem(idx, "delta_quantity", e.target.value)}
                        className="w-full rounded-xl border-slate-200 dark:bg-slate-900 text-sm text-right"
                        placeholder="0"
                    />
                    {errors[`items.${idx}.delta_quantity`] && (
                        <p className="text-red-500 text-xs mt-1">{errors[`items.${idx}.delta_quantity`]}</p>
                    )}
                </div>
            </div>
            ...
            {item.delta_quantity !== "" && (
                <div className={`flex flex-col xs:flex-row items-start xs:items-center justify-between gap-1 px-3 py-2.5 rounded-xl text-xs sm:text-sm font-bold border ${
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <span>Qty Akhir: {fmtQty(finalQty)} {ing?.unit ?? "unit"}</span>
                    </div>
                    <span className="text-xs font-semibold opacity-80">Nilai Selisih: {fmt(valDiff)}</span>
                </div>
            )}
...
export default function Create({ warehouses, stores, ingredients, packagingMaterials, typeOptions }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        notes:           "",
        is_delta:        true,
        items: [{ item_type: "ingredient", item_id: "", system_quantity: 0, delta_quantity: "", unit_cost: 0.0, notes: "" }],
    });
    ...
    const addItem    = () => setData("items", [...data.items, { item_type: "ingredient", item_id: "", system_quantity: 0, delta_quantity: "", unit_cost: 0.0, notes: "" }]);
        ...
        <>
            ...
            <div className="max-w-5xl mx-auto">
                ...
                <form onSubmit={submit} className="space-y-4 sm:space-y-6">
                    ...
                    {/* Header */}
                    <div className="bg-gradient-to-r from-teal-500 to-teal-600 text-white rounded-2xl p-4 sm:p-6 shadow-lg">
                        <h2 className="text-xl sm:text-2xl font-bold flex items-center gap-2">
                            <IconAdjustments size={24} className="shrink-0" /> Buat Delta Adjustment
                        </h2>
                    ...
                    <div className="p-3 sm:p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 flex gap-2 sm:gap-3">
                        ...
                        <p className="text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
                            Sistem otomatis mengambil stok saat ini. Masukkan <strong>delta (+ atau -)</strong> untuk menambah atau mengurangi stok.
                        </p>
                    </div>
