...
import {
    IconArrowLeft, IconDeviceFloppy, IconFlask, IconPackage,
    IconPhoto, IconCoin, IconInfoCircle, IconX, IconGift, IconCurrencyDollar,
...
function PriceInput({ label, value, onChange, hint, errors, disabled = false }) {
    ...
    const handleChange = (e) => setDisplay(e.target.value.replace(/[^\d.,]/g, ""));
    const handleBlur = () => onChange(Number(parseDecimal(display)));
...
export default function Create({ material_type, categories, sizes }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        code:                   "",
        name:                   "",
        unit:                   isKemasan ? "pcs" : "ml",
        size_id:                "",
        image:                  null,
        description:            "",
        selling_price:          isKemasan ? 0 : "",
        purchase_price:         0,
        is_free:                false,
        free_condition_note:    "",
    ...
    const selectedCategory = categories.find(c => String(c.id) === String(data.material_category_id));
    ...
    const submit = (e) => {
        ...
        post(route("materials.store"), {
            forceFormData: true,
            onSuccess: () => toast.success(isKemasan ? "Material kemasan berhasil disimpan" : "Bahan baku berhasil ditambahkan"),
            onError:   () => toast.error("Periksa kembali form Anda"),
        });
    };

    const effectivePrice = data.is_free ? 0 : data.selling_price;
    const margin = isKemasan && !data.is_free && effectivePrice > 0 && data.purchase_price > 0
        ? (((effectivePrice - data.purchase_price) / effectivePrice) * 100).toFixed(2)
        : null;
    const subsidyPerUnit = isKemasan && data.is_free && data.purchase_price > 0 ? data.purchase_price : null;

    const backRoute = route("materials.index", { material_type });
    const title = isKemasan ? "Tambah Kemasan" : "Tambah Bahan Baku";
    ...
    return (
        <>
            ...
            <div className="max-w-5xl mx-auto px-4 py-6">
                ...
                >
                    <IconArrowLeft size={16} /> Kembali ke Daftar {isKemasan ? "Kemasan" : "Bahan Baku"}
                </Link>
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-0.5">
                            {isKemasan ? "Tambahkan material kemasan baru ke inventaris" : "Isi data bahan baku baru untuk digunakan dalam formula parfum"}
                        </p>
                    </div>
                </div>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                                <h2 className="text-base font-bold mb-5 flex items-center gap-2 text-slate-800 dark:text-white">
                                    <Icon size={18} className="text-teal-600" /> Informasi Dasar
                                </h2>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <Input
                                        ...
                                        errors={errors.code}
                                        placeholder={isKemasan ? "BTL-50ML-001" : "OIL-LAV-01"}
                                    />
                                    <div>
                                        <Select
                                            ...
                                            errors={errors.material_category_id}
                                        >
                                ...
                                <div className="mb-4">
                                    <Input
                                        label={isKemasan ? "Nama Kemasan" : "Nama Bahan Baku"} required
                                        value={data.name}
                                        onChange={e => setData("name", e.target.value)}
                                        errors={errors.name}
                                        placeholder={isKemasan ? "Botol Spray 50ml" : "Fragrance Oil Lavender"}
                                    />
                                </div>
                        ...
                        <div className="space-y-5">
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
                                ...
                                <div
                                    ...
                                    onClick={() => document.getElementById('mat-img-create').click()}
                                >
                                    {preview ? (
                                        <>
                            ...
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
                                {(isKemasan ? [
                                    ...
                                    { key: "is_active", label: "Status Aktif", desc: "Bahan muncul di semua menu" },
                                ]).map(({ key, label, desc }) => (
                            ...
                            >
                                ...
                                {processing ? "Menyimpan..." : (isKemasan ? "Simpan Kemasan" : "Simpan Bahan Baku")}
                            </button>
