...
import {
    IconArrowLeft, IconDeviceFloppy, IconPhoto, IconFlask, IconX,
    IconCoin, IconInfoCircle, IconLock, IconPackage, IconGift, IconCurrencyDollar,
...
export default function Edit({ material, categories, sizes }) {
    const isKemasan = material.material_type === "bahan_kemasan";
    const [preview, setPreview] = useState(material.image_url ?? null);
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        code:                   material.code              || "",
        name:                   material.name              || "",
        unit:                   material.unit              || (isKemasan ? "pcs" : "ml"),
        size_id:                material.size_id           || "",
        image:                  null,
        description:            material.description       || "",
        selling_price:          material.selling_price     ?? (isKemasan ? 0 : ""),
    ...
    const avgCost = parseFloat(material.average_cost || 0);
    const effectivePrice = data.is_free ? 0 : data.selling_price;
    const margin = isKemasan && !data.is_free && effectivePrice > 0 && avgCost > 0
        ? (((effectivePrice - avgCost) / effectivePrice) * 100).toFixed(2)
        : null;
    const subsidyPerUnit = isKemasan && data.is_free && avgCost > 0 ? avgCost : null;

    const backRoute = route("materials.index", { material_type: material.material_type });
        ...
        <>
            ...
            <div className="max-w-5xl mx-auto px-4 py-6">
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800 dark:text-white">{isKemasan ? "Edit Kemasan" : "Edit Bahan Baku"}</h1>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
                                <h2 className="text-base font-bold mb-5 text-slate-800 dark:text-white">
                                    Ubah Informasi {isKemasan ? "Kemasan" : "Bahan Baku"}
                                </h2>
                                    ...
                                    <div className="bg-teal-50 dark:bg-teal-950/20 border border-teal-100 dark:border-teal-900 rounded-xl p-4 flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-teal-700 dark:text-teal-400">
                                                Otomatis terhitung via metode <strong>Weighted Average Cost (WAC)</strong> setiap
                                                kali Purchase Order diterima. Tidak bisa diisi manual.
                                            </p>
                                        </div>
                                    </div>
                                </>
                        ...
                        <div className="space-y-5">
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
                                ...
                                <div
                                    ...
                                    onClick={() => document.getElementById('mat-img-edit').click()}
                                >
                                    {preview ? (
                                        <>
                            ...
                            {isKemasan && data.is_assembly && (
                                <Link
                                    href={route("materials.components.edit", material.id)}
