            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold dark:text-white flex items-center gap-2">
                        <IconUsers className="text-slate-700" /> Tenaga Penjual
                    </h1>
                </div>
                        ...
                        {salesPeople.data.map((s) => (
                            <tr key={s.id}>
                                ...
                                <Table.Td className="text-center">
                                    <span className={`px-2 py-1 rounded text-[10px] font-bold bg-slate-100 text-slate-700 border border-slate-300`}>
                                        {s.is_active ? 'AKTIF' : 'NON-AKTIF'}
                                    </span>
                                </Table.Td>
                                <Table.Td>
                                    <div className="flex gap-2">
                                        <Link href={route('sales-people.targets', s.id)} className="p-2 bg-slate-100 text-slate-700 rounded-lg" title="Set Target Bulanan">
                                            <IconTargetArrow size={18} />
                                        </Link>
                                        <Button type="edit" href={route('sales-people.edit', s.id)} icon={<IconPencilCog size={16} />} className="bg-slate-100 text-slate-700" />
                                        <Button type="delete" url={route('sales-people.destroy', s.id)} icon={<IconTrash size={16} />} className="bg-slate-100 text-slate-700" />
                                    </div>
                                </Table.Td>
                            </tr>
                        ))}
